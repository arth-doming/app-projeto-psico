  ...
  void _deletarPaciente() {
    showDialog(
      ...
      builder: (_) => AlertDialog(
        ...
        content: Text('Deseja deletar ${_paciente.nome}? Esta ação não pode ser desfeita.'),
        actions: [
          TextButton(
          ...
          TextButton(
            onPressed: () async {
              await EvolucaoService.deletarPorPaciente(_paciente.id);
              await PacienteService.deletar(_paciente.id);
              if (mounted) {
        ...
        child: Column(
          ...
          children: [
            Row(children: [
              ...
              Text('Nova evolução — ${_paciente.nome.split(' ').first}',
                  style: const TextStyle(fontSize: 17, fontWeight: FontWeight.bold)),
            ]),
            const SizedBox(height: 6),
            Text(DateFormat("d 'de' MMMM 'de' yyyy", 'pt_BR').format(DateTime.now()),
          ...
          child: ListView(
            ...
            children: [
              ...
              Row(
                ...
                children: [
                  Text(DateFormat("d 'de' MMMM 'de' yyyy", 'pt_BR').format(e.data),
                      style: const TextStyle(fontWeight: FontWeight.bold,
                          fontSize: 16, color: Color(0xFF7C6FCD))),
                  IconButton(
                    icon: const Icon(Icons.delete_outline, color: Colors.red),
                    onPressed: () async {
                      await EvolucaoService.deletar(e.id);
                      if (mounted) Navigator.pop(context);
                    },
                  ),
                ],
              ...
              ElevatedButton(
                onPressed: () async {
                  if (controller.text.trim().isEmpty) return;
                  await EvolucaoService.atualizar(Evolucao(
                    id: e.id, pacienteId: e.pacienteId,
                    pacienteNome: e.pacienteNome, data: e.data,
                    anotacao: controller.text.trim(), consultaId: e.consultaId,
                  ));
                  if (mounted) Navigator.pop(context);
                },
                style: ElevatedButton.styleFrom(
                ...
                child: const Text('Salvar edição', style: TextStyle(fontSize: 16)),
              ),
            ],
          ),
        ),
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      body: StreamBuilder<List<Evolucao>>(
        stream: EvolucaoService.listarPorPaciente(_paciente.id),
            ...
            children: [
              Center(
                child: CircleAvatar(
                  radius: 42,
                  backgroundColor: const Color(0xFF7C6FCD),
                  child: Text(_paciente.iniciais,
                      style: const TextStyle(color: Colors.white,
                          fontSize: 28, fontWeight: FontWeight.bold)),
                ),
              ),
              const SizedBox(height: 12),
              Center(child: Text(_paciente.nome,
                  style: const TextStyle(fontSize: 20, fontWeight: FontWeight.bold))),
              Center(child: Text('${_paciente.idade} anos',
              ...
              _infoTile(Icons.cake, 'Nascimento',
                  DateFormat("d 'de' MMMM 'de' yyyy", 'pt_BR').format(_paciente.dataNascimento)),
                ...
                children: [
                  ...
                  if (evolucoes.isNotEmpty)
                    Padding(
                      ...
                      child: Text('${evolucoes.length} registro(s)',
                          style: TextStyle(fontSize: 13, color: Colors.grey[500])),
                    ),
                ],
              ),
              if (snapshot.connectionState == ConnectionState.waiting)
                ...
                ...evolucoes.map((e) => _evolucaoCard(e)),
            ...
            Row(children: [
              ...
              Text(DateFormat("d 'de' MMMM 'de' yyyy", 'pt_BR').format(e.data),
                  style: const TextStyle(fontSize: 13,
                      fontWeight: FontWeight.w600, color: Color(0xFF7C6FCD))),
              const Spacer(),
              Icon(Icons.chevron_right, size: 18, color: Colors.grey[400]),
            ]),
            const SizedBox(height: 10),
            Text(e.anotacao, maxLines: 3, overflow: TextOverflow.ellipsis,
